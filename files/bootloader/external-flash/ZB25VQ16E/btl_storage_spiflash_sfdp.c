...
static uint8_t SFDP_getManufactID(void)  //Device Manufacturer ID
{
  uint8_t id;
...
{
  uint8_t id = SFDP_getManufactID();

  if (id == 0xBA || id == 0xB3) { // // ZETTA
    return 80; //ms
  }
...
static uint32_t SFDP_getSectorSize(void)
{
  ...
  uint8_t id = SFDP_getManufactID();
  #ifdef D_ZB25VQ16C
  if (id == 0x5E) { // Zbit manufacture ID, the sector size is 4KB
	offset = 0x18;
  }
  #endif
  spi_setCsActive();
...
static uint32_t SFDP_eraseSize(void)
{
  ...
  uint8_t id = SFDP_getManufactID();
  if (id == 0xBA || id == 0xB3) { // ZETTA
    type4 = 0; // For ZETTA chipset, the type4 is page_size(256byte), type3 is block_size(64KB), we shold return the max size can be erasing
  #ifdef D_ZB25VQ16C
  } else if (id == 0x5E) {
    type2 = 0; // For Zbit chipset, the page_size(256byte), block_size(64KB), we shold return the max size can be erasing
    type3 = 0;
    type4 = 0;
  #endif
  }
  // ZB25VQ16E follows standard SFDP layout:
  // type1 = 4KB (0Ch), type2 = 32KB (0Fh), type3 = 64KB (10h), type4 = not supported (00h)
  // No special handling needed - the function will correctly return 64KB (type3)
