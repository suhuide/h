...
> **记住两个"只能有 0 或 1 个"**：PAI/DAC 的 PID 是 X.509 DN 里的单值 RDN，**天生不支持列表**。想要"一个中间证书覆盖多个型号"，唯一的做法是 PAI 不带 PID + CD 的 product_id_array 列出全部已认证 PID（见 2.5 常见问题）。
...
### 2.4.1 实测样例：Test PAA / PAI / DAC 逐字段解读

下面是同一套测试 PKI 的三张证书 `openssl x509 -text` 原始输出：Test PAA 01（自签）→ PAI 01 → DAC（VID 0x1470 + PID 0xF100），与 2.2 节的量产证书族（PID 0x8006）互为对照。每张后面的"要点"对应上表的检查项。

**PAA —— HOPERF Matter Test PAA 01**

```text
$ openssl x509 -noout -in HopeRFTestPAACertificate.pem -text
Certificate:
    Data:
        Version: 3 (0x2)
        Serial Number:
            44:ea:cb:64:5c:a6:12:b2:ff:37:dd:b9:70:52:75:b0
        Signature Algorithm: ecdsa-with-SHA256
        Issuer: CN = HOPERF Matter Test PAA 01, 1.3.6.1.4.1.37244.2.1 = 1470
        Validity
            Not Before: Aug 22 05:23:12 2023 GMT
            Not After : Jul  5 06:23:12 2223 GMT
        Subject: CN = HOPERF Matter Test PAA 01, 1.3.6.1.4.1.37244.2.1 = 1470
        Subject Public Key Info:
            Public Key Algorithm: id-ecPublicKey
                Public-Key: (256 bit)
                pub:
                    04:33:d0:e5:07:80:e7:da:aa:0e:65:a7:8f:c3:7b:
                    6b:bd:1f:3f:d4:3e:e3:f5:85:46:2c:34:74:a5:91:
                    e1:0d:3d:f7:5d:65:df:9a:0c:6d:d3:e4:d1:a1:78:
                    a2:fa:cc:65:24:1f:4d:dc:9a:13:db:09:8b:6b:84:
                    67:b8:2d:78:a4
                ASN1 OID: prime256v1
                NIST CURVE: P-256
        X509v3 extensions:
            ...
            X509v3 Subject Key Identifier:
                FE:10:B5:6C:76:D3:9E:16:8A:F7:48:F4:07:95:D8:EA:A6:F8:98:8F
    ...
    Signature Algorithm: ecdsa-with-SHA256
    Signature Value:
        30:45:02:21:00:ce:cf:33:6f:fa:1b:b1:5d:4b:41:38:b6:b7:
        01:5e:e8:4a:9f:f2:dc:75:23:d5:01:59:4d:4e:8a:74:1b:d9:
        81:02:20:42:6f:fa:37:da:70:85:ed:c9:b1:7f:a0:10:ab:f1:
        fe:9e:24:e9:f0:2f:47:f2:18:40:37:1f:86:c2:26:b7:76
```

要点：`CA:TRUE` 且**未带 pathLen**——规范允许"缺省或 1"两种写法，这张用的是缺省写法；KeyUsage = digitalSignature + keyCertSign + cRLSign（digitalSignature 是 MAY 位，合法）；SKID == AKID（自签根标志）；Subject 只有 VID（0x1470），无 PID。

**PAI —— HOPERF Matter PAI 01**

```text
$ openssl x509 -noout -in HopeRFTestPAICertificate.pem -text
Certificate:
    Data:
        Version: 3 (0x2)
        Serial Number:
            55:3a:22:50:7d:2e:cd:de:21:4a:10:f4:b3:89:c6:01
        Signature Algorithm: ecdsa-with-SHA256
        Issuer: CN = HOPERF Matter Test PAA 01, 1.3.6.1.4.1.37244.2.1 = 1470
        Validity
            Not Before: Aug 24 02:12:42 2023 GMT
            Not After : Apr 20 03:12:42 2215 GMT
        Subject: CN = HOPERF Matter PAI 01, 1.3.6.1.4.1.37244.2.1 = 1470
        Subject Public Key Info:
            Public Key Algorithm: id-ecPublicKey
                Public-Key: (256 bit)
                pub:
                    04:7c:7d:80:10:0c:4b:d8:66:60:bd:cd:13:83:31:
                    0a:93:dc:12:1d:b0:67:fa:e2:52:09:e3:c8:94:91:
                    74:d0:80:47:d8:28:79:fc:4f:e9:fd:a5:51:fa:1a:
                    f1:e4:30:14:b8:2d:53:56:6d:f4:b4:6b:e6:a4:1d:
                    9e:21:3b:09:e3
                ASN1 OID: prime256v1
                NIST CURVE: P-256
        X509v3 extensions:
            ...
            X509v3 Authority Key Identifier:
                FE:10:B5:6C:76:D3:9E:16:8A:F7:48:F4:07:95:D8:EA:A6:F8:98:8F
            X509v3 Subject Key Identifier:
                2F:9D:7D:4F:61:B5:56:90:68:D9:9C:8C:85:89:5D:82:BC:AC:34:FD
            X509v3 Key Usage: critical
                Certificate Sign, CRL Sign
    Signature Algorithm: ecdsa-with-SHA256
    Signature Value:
        30:45:02:21:00:c8:4f:35:e0:e2:26:70:4d:69:81:8c:8d:bc:
        d6:1c:5b:85:2a:a9:d8:3c:8a:5b:31:fd:b0:97:6c:2d:77:1d:
        3b:02:20:43:5c:0d:db:26:ab:d1:bb:70:2d:22:12:ad:b7:44:
        d1:bf:b5:59:d9:14:bb:f0:a5:e1:8c:f6:ed:3d:fb:42:d2
```

要点：`CA:TRUE, pathlen:0`（PAI 强制为 0，不允许缺省）；KeyUsage = Certificate Sign + CRL Sign，两位齐全；AKID == PAA 的 SKID（`FE:10:...`）；Issuer 与 PAA 的 Subject 逐字节相同；Subject 有 VID 无 PID（服务该 VID 下所有产品）。

**DAC —— HOPERF Matter DAC**

```text
$ openssl x509 -noout -in DAC/data/test_dac-cert.pem -text
Certificate:
    Data:
        Version: 3 (0x2)
        Serial Number:
            e6:6f:d3:57:0d:db:68:b8:cb:a1:09:2d:7c:1b:d3:0b
        Signature Algorithm: ecdsa-with-SHA256
        Issuer: CN = HOPERF Matter PAI 01, 1.3.6.1.4.1.37244.2.1 = 1470
        Validity
            Not Before: Aug 24 02:17:48 2023 GMT
            Not After : Jul 31 03:17:48 2123 GMT
        Subject: CN = HOPERF Matter DAC, 1.3.6.1.4.1.37244.2.1 = 1470, 1.3.6.1.4.1.37244.2.2 = F100
        Subject Public Key Info:
            Public Key Algorithm: id-ecPublicKey
                Public-Key: (256 bit)
                pub:
                    04:c4:33:2f:51:a3:dc:9f:e9:0b:8c:af:92:44:6c:
                    31:07:a3:bc:6c:43:1e:18:48:ef:cc:b9:39:c9:67:
                    d2:2f:76:18:bd:63:5f:b8:94:0b:51:53:8d:f4:5b:
                    7d:8e:d4:7d:23:af:c1:7b:c0:b9:6a:ec:cf:76:3b:
                    20:21:c1:66:f5
                ASN1 OID: prime256v1
                NIST CURVE: P-256
        X509v3 extensions:
            ...
            X509v3 Authority Key Identifier:
                2F:9D:7D:4F:61:B5:56:90:68:D9:9C:8C:85:89:5D:82:BC:AC:34:FD
            X509v3 Subject Key Identifier:
                BA:48:65:07:B5:85:E6:30:23:C6:59:E4:6B:9E:92:AC:A8:E4:AE:7F
            X509v3 Key Usage: critical
                Digital Signature
    Signature Algorithm: ecdsa-with-SHA256
    Signature Value:
        30:45:02:21:00:b6:91:1f:a8:c0:2d:23:28:51:73:1c:96:36:
        c8:17:0a:40:44:dc:0c:54:b0:f2:62:7c:95:12:63:06:33:89:
        3a:02:20:29:de:d9:94:50:1e:5c:75:6c:63:de:8b:1d:b4:a2:
        77:c9:b7:69:ec:e6:ff:4c:39:d1:65:9c:7c:44:9a:1e:91
```

要点：`CA:FALSE`（无 pathLen）；KeyUsage **仅 digitalSignature**；AKID == PAI 的 SKID（`2F:9D:...`）；Issuer 与 PAI 的 Subject 逐字节相同；Subject = VID（`1470`）+ PID（`F100`）双必选——注意 PID 值是 4 字符大写 HEX 字符串 `F100`，前导零规则见 3.2 节。

三张连起来正好走通第 2.2 节的三条硬性规定：链恰好 3 层、PAI 归属 VID 且不带 PID、整链 DER 均 ≤ 600 字节、SKID/AKID 首尾相接（`FE:10:... → 2F:9D:... → BA:48:...`）。


### 2.5 常见问题：PAI 的 PID 限制

**问：PAI 能不能带"一组 PID"，让 DAC 报列表里任意一个都通过？**
...
### 4.5 常见问题：签名为什么要同时掺 nonce 和 challenge

**问：签名为什么要同时掺 nonce 和 challenge？**
...
### 5.7 常见问题：CD 校验中的 VID/PID 交叉验证

以下四个问题都围绕 CD 的 VID/PID 交叉校验规则。

**问：DAC 的 PID 不在 CD 的 product_id_array 里，能入网吗？**

不能，这是硬性失败：SDK 返回 `kCertificationDeclarationInvalidProductId(605)`，规范 §6.2.3.1 为 SHALL 级校验，chip-tool 直接终止 commissioning。同理 VID 不匹配报 604。唯一例外是 ODM 模式（见下文 dac_origin 一问）。


**问：那如果真有一台"PID 对不上却入网成功"的设备，怎么解释？**
...
**问：dac_origin_vendor_id / dac_origin_product_id 是什么？（"Original PID"）**
...
- **实例**：日志中设备 Basic Info 报 `vendorId=0x149A productId=0x3005`，DAC 携带 `0x1470/0x8006`，CD 打印 `VID: 0x149A` + `DAC origin VID: 0x1470, PID: 0x8006` 且 `certification_type: 2 (Certified device)`——一条日志看全 ODM 模式的所有角色（详见 6.8 日志注解）。


**问：Basic Information 集群报的 VID/PID 和证书不一致会怎样？**
...
## 6. 设备端落地（Silicon Labs）

### 6.1 DAC 与 NOC：出厂身份 vs 入网身份
...
### 6.2 统一入口：DeviceAttestationCredentialsProvider
...
新布局按 flash 页（0x1000）对齐，各段之间留出隔离区；旧布局紧贴基址排布，一旦索引与 flash 实际内容版本不同步，就容易读到已擦除区域（见 6.4 案例）。索引实测：
...
**问：开发用 0xFFF1/0x8000 测试证书，量产要注意什么？**
...
### 6.8 量产设备日志逐段注解（chip-tool BLE + Thread）

> 完整日志：`commissioning/commissioning-raspi-log.md`。这是 chip-tool（Commissioner 控制台）的完整配网记录，是设备侧行为的镜像：证书与 CD 由设备上报、签名由设备私钥生成，不是 0xFFF1 测试件——**CD 走的是 ODM（dac_origin）模式，认证类型为正式认证**，是本档所有规则的一次完整落地。
...
### 7.1 AttestationVerificationResult 全表（SDK 真实值）
...
1. 拿到错误码数值，按下表分段定位到验证阶段（100=PAA、200=PAI、300=DAC、500=attestation、600=CD）；
2. 凭据类问题用 `commander nvm3 read / parse` 回读设备凭据区，核对索引与 flash 内容（见 6.3）；
3. 对照 6.8 的日志逐段注解，确认卡在哪个 commissioning stage、哪一行输出异常；
4. 规则类问题（VID/PID 编码、CD 交叉、ODM 模式）先复核第 2/3/5 章的表格与矩阵。
...
| 现象（错误码） | 常见根因 | 排查动作 |
|---------------|---------|---------|
| `kPaaNotFound(101)`，日志 `Unable to find PAA ... PAI's AKID: xx` | 信任库缺 PAA：`--paa-trust-store-path` 目录不对 / PAA 未注册 / AKID 与库内 SKID 不匹配 | 核对信任库目录与 DCL 注册；确认 AKID 与库内 PAA 的 SKID 一致 |
| `kDacVendorIdMismatch(305)` | DAC 与 PAI 的 VID 不一致，或 PAI 缺 VID | 回读两证书 DN（`openssl x509 -text`），对照 §2.4 的 Subject 要求 |
| `kDacProductIdMismatch(306)` | DAC 缺 PID，或 PAI.PID 与 DAC.PID 不等 | 回读证书 DN；PAI 带 PID 的产线要保证与 DAC 同 PID |
| `kDacSignatureInvalid(301)` | 链签名失败：PAI 不是签发该 DAC 的钥匙（常为产线刷错 PAI） | 按 6.4 回读凭据区，核对 AKID/SKID 链；整链重烧 |
| `kAttestationSignatureInvalid(500)` | 设备端签名数据/密钥不对（elements、challenge 拼接顺序，或没用 DAC 私钥） | 检查 `SignWithDeviceAttestationKey` 的输入拼接（见 4.1/6.2） |
| `kAttestationNonceMismatch(502)` | 设备没把请求里的 nonce 原样放进 elements[2] | 抓设备侧 elements TLV，核对 nonce 原样回填 |
| `kCertificationDeclarationInvalidProductId(605)` | DAC/Basic Info 的 PID 不在 CD 列表（见 5.7） | 按 5.5 矩阵核对证书 ↔ CD ↔ Basic Info 三处同源 |
| `kCertificationDeclarationNoCertificateFound(601)` + `Disallowing CD signed by test key` | CD 用测试钥匙签发，而 Commissioner 已关测试钥匙支持 | 换正式 CD（见 5.4 六把钥匙、6.7 量产切换） |

### 7.3 日志关键信息提取技巧

拿到一份 chip-tool 日志时，按下面的关键字快速定位（完整逐段注解见 6.8）：

| 要确认什么 | 在日志里找 |
|-----------|-----------|
| 证书链是否齐全、链接是否正确 | `DAC/PAI/PAA certificate SKID/AKID` 各行——AKID 应逐级等于上级 SKID |
| 设备身份（VID/PID 交叉） | `--> DAC's VID: 0x..., PID: 0x...` |
| CD 用哪把钥匙验签 | `CD signing key identifier:`（对照 5.4 的六把钥匙表） |
| CD 交叉校验结果 | `--> VID / Device type ID / Certification type / DAC origin VID` |
| 流程卡在哪个阶段 | `Commissioning stage: -> '...'` 与 `Successfully finished commissioning step '...'` |
| 吊销检查是否执行 | `WARNING: No revocation delegate available...` |
| 信任库查找失败 | `Unable to find PAA, err: ..., PAI's AKID: ...` |
