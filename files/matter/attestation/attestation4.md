...
> 三张证书与 §5 配网日志中设备上报的内容完全一致,可对照阅读.
...
三级证书 Subject DN 里 VID/PID 的要求(Spec v1.5 原文核对,章节号与 v1.2 一致):
...
把它和 §2 设备侧的 `paa.pem` 逐项对比——**是同一张证书**:  

| 字段 | 信任库 dcld_mirror_*.pem | 设备侧 paa.pem (§2) | 比对 |
|------|--------------------------|----------------------|------|
