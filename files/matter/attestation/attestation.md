...
Attestation 验证通过后,Commissioner 给设备发"运营身份".  
这一阶段设备要做一件大事:**现场生成一把全新的 NOC 密钥对**(与 DAC 密钥对无关)——运营身份与出厂身份从此解耦.

```mermaid
sequenceDiagram
    autonumber
    participant C as Commissioner (chip-tool)
    participant D as 设备

    rect rgb(255, 246, 232)
    Note over C,D: ① CSRRequest{(0x04)  (nonce 32B) }
    Note right of D: 设备现场生成 NOC 密钥对(私钥留下)<br/>组装 NOCSR Elements(TLV):<br/>tag1 = CSR(PKCS#35;10 DER, 含新公钥)<br/>tag2 = attestation_nonce(抄写)<br/>attestation_signature = DAC私钥( SHA256(NOCSR ‖ challenge) )
    D-->>C: ② CSRResponse{(0x05)  (NOCSR Elements + attestation_signature) }
    Note over C: ③ ValidateCSR:用 DAC 公钥验 attestation_signature<br/>→ 证明 CSR 确实来自这颗 DAC 设备, 未被调包
    Note over C: ④ GenerateNOCChain:签发运营证书<br/>NOC.Subject = Node ID + Fabric ID + CSR 里的新公钥<br/>(chip-tool 默认: RCAC 现场自签, 无 ICAC)
    end

    rect rgb(232, 250, 236)
    Note over C,D: 安装运营身份
    C->>D: ⑤ AddTrustedRootCertificate{(0x11)  RCAC }   ← 先发根, 设备才能验 NOC
    C->>D: ⑥ AddNOC{(0x06)  (NOC(DER), IPK, caseAdminSubject) }
    D-->>C: ⑦ NOCResponse{(0x08)  (statusCode = OK, fabricIndex = 1) }
    Note right of D: 设备把 NOC 写入 fabric 表<br/>运营身份生效(日志: CASE establishment successful)
    end
```
```c
NOCSR Elements (TLV)
 ├─ tag 1: PKCS#10 CSR（新运营公钥）
 ├─ tag 2: attestation_nonce (32B, Commissioner 现发)
 └─ (厂商自定义 tag 可选)
外附: attestation_signature = DAC私钥( SHA256(NOCSR ‖ challenge) )
```
```c
Root Certificate Authority Certificate——运营侧的根 CA 证书.Matter 的运营证书链是：
RCAC（根）→ [ICAC 中间，可选] → NOC（叶子，设备身份）
```
**日志对照** `SendOpCertSigningRequest → ValidateCSR →
GenerateNOCChain → SendTrustedRootCert → SendNOC`(期间出现 `CASE establishment successful` /
`Secure Pairing Success`).

**两个密钥对,两种身份:**

| | **DAC 密钥对**(出厂身份) | **NOC 密钥对**(运营身份) |
|---|---|---|
| 何时产生 | 产线(烧录时) | **每次配网时现场生成** |
| 私钥位置 | PSA key, 永不导出 | 设备 fabric 存储 |
| 证书由谁签 | PAI(链条到 PAA) | Commissioner 的 RCAC |
| 用途 | 仅配网: 证明"我是正品"(两次签名) | 配网完成后**所有日常通信**(CASE) |
| 生命周期 | 设备终身不变(恢复出厂也不变) | 换 fabric / 转卖设备 → 整套重生成 |

**为什么 NOC 阶段要生成新密钥对, 而不直接用 DAC 私钥?**  
身份隔离.  
DAC 证明"这台设备是谁生产的", NOC 证明"这台设备在这个 fabric 里叫什么 Node ID".  
两者解耦后: 设备可以加入多个 fabric(每个 fabric 一套 NOC+密钥), 转卖/重配时 DAC 原封不动, 旧fabric的运营身份随NOC一起删除.  
