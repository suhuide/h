...
> **补充记录（2026-08-20, aok02_matter_ac）**: 新增订阅恢复失败路径实测分析（0x7d InvalidSubscription → torn down），见 §2.4.5 / §6.6，完整日志见 [recovery/log-subscriptions-resume.md](recovery/log-subscriptions-resume.md)。
...
#### 2.4.5 订阅恢复失败路径（0x7d InvalidSubscription → torn down）— aok02 实测补充

> **实测来源**: aok02_matter_ac 固件 Ver 10000，2026-08-20。完整日志: [recovery/log-subscriptions-resume.md](recovery/log-subscriptions-resume.md)

上文 §2.4.4 描述的是订阅恢复**成功**的路径。实测中存在一条必须单独说明的**失败路径**：
**CASE 恢复成功、但 priming report 被控制器以 0x7d（InvalidSubscription）拒绝，订阅被拆除。**

**实测时间线**（设备断电再上电）：

```
00:00:14.547  Resuming 1 subscriptions in 0 seconds       ← 设备主动恢复订阅（PERSIST_SUBSCRIPTIONS=1）
00:00:14.769  设备主动发 CASE_Sigma1（设备是 Initiator）
00:00:14.833  Received Sigma2Resume                        ← ✅ CASE 恢复成功（仅 0.3s）
00:00:14.848  Registered a ReadHandler                     ← 订阅对象已建立
00:00:15.097  Received status response, status is 0x7d     ← ❌ 控制器对 priming report 回 InvalidSubscription
00:00:15.108  Subscription id 0x21bcb30d ... torn down     ← 设备按协议拆除订阅，持久化记录被删除
---（等待窗口：设备侧没有任何动作可做）---
00:00:33.442  控制器（hub）主动发 CASE_Sigma1              ← 用户在 APP 上点操作触发
00:00:33.649  CASE session established（新会话 + 新订阅）
00:00:38.810+ priming reports 全部 0x00 确认               ← 恢复正常
```

**关键结论**:

1. **0x7d = 控制器侧丢失了订阅状态**（hub 重启 / 状态清理 / 多 hub 迁移）。设备按 Spec §8.5.3 拆除订阅并删除持久化记录——此后双方都不再持有该订阅，不存在可恢复的对象。
2. **订阅只能由控制器（订阅方）发起 SubscribeRequest 重建**。常电非 ICD 设备没有任何协议消息可以"请求控制器来订阅我"——Check-In（0x50）是 ICD 专属机制（本设备 `CHIP_CONFIG_ENABLE_ICD_CIP=0`，不适用，见 §1.3）。
3. 设备主动发起 CASE 本身无意义：没有订阅的空会话不会促使控制器重新订阅。
4. 因此"点一下 APP 才能恢复"的根因在**控制器侧**的发现/重订阅策略，设备端无协议内手段强制加速。

**设备端可选的间接助推**（均为网络层动作，不是协议消息）：

| 方案 | 做法 | 风险 | 评价 |
|------|------|------|------|
| 温和助推 | 检测到订阅 torn down 后 N 秒内无新 CASE → Thread detach+reattach 一次（`otThreadSetEnabled(false/true)`），迫使 SRP 重注册 + mDNS 重公告，可能提前唤醒控制器发现监听 | 打断其他控制器的既有会话/订阅；只能在"拆除后、重连前"窗口执行 | 可选实验，未实施 |
| DNS-SD TXT boot counter | 每次重启 TXT 记录变化，逼 mDNS 立即重公告 | 非标准 vendor key、侵入大、对主动重查的控制器无感 | 不建议 |
| 等待自愈 | 不做任何事 | — | 控制器通常 30s~2min 内自发重连 |

**责任边界判定**（用日志定位"卡住"在哪一侧）：

| 设备侧日志表现 | 结论 |
|----------------|------|
| 重启后 1-3s 内完成 Resuming → Sigma2Resume → ReadHandler → priming report 0x00 确认 | 设备侧 100% 完成；APP 仍显示离线 = 控制器侧问题，设备代码无解 |
| priming report 收到 0x7d → torn down | 控制器侧丢订阅状态（本路径）；等待控制器重新订阅，或按上表做温和助推 |
| `rejoin failed (N), restart Thread stack` 反复出现 | 网络层未恢复，§6.1 入网重试在工作 |
| 某个 peer 的 DNS 解析挂起 / Sigma 无响应 | SDK 持久化订阅自带重试调度，观察 `Resuming N subscriptions` 重试日志即可 |

**已落地的配套优化**（aok02_matter_ac，2026-08-20）: 入网失败重试（§6.1 思路落地）+ 上电标记关键属性 dirty（保证订阅恢复后 priming report 一次带全最新值，免控制器补一轮 Read）。
...
### 6.6 订阅恢复失败后的处置（aok02 实测补充，2026-08-20）

详见 §2.4.5（完整日志: [recovery/log-subscriptions-resume.md](recovery/log-subscriptions-resume.md)）。要点：

1. **入网失败重试（§6.1 思路）已落地**：`_rejoin_timeout_event_handler` 超时后重启 Thread 栈（`otThreadSetEnabled(false/true)`）并无限重试，满足 §11.8.4 至少 5 分钟要求；`kConnectivity_Lost` 也启动同一看门狗。
2. **上电标记关键属性 dirty 已落地**：灯/插座 Init 后调用 `MatterReportingAttributeChangeCallback`（OnOff、CurrentLevel、RGB 的 Hue/Sat/X/Y），保证订阅恢复后 priming report 一次带全，免控制器补一轮 Read。
3. **订阅 torn down（0x7d）后设备端无可恢复状态**，只能等控制器重新订阅；"温和助推"（detach+reattach 逼 mDNS 重公告）为可选实验项，未实施。是否实施取决于实测的自愈时长：若控制器通常在 2 分钟内自发重连，则无需任何代码。
